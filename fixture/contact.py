...
class ContactHelper:
    ...
    def get_contact(self, index):
        wd = self.app.wd
        # Assume we are on home page already
        ids = wd.find_elements_by_name("selected[]")
        if index in range(0, len(ids)):
            return ids[index]
        return wd.find_element_by_name("selected[]")

    def modify_field(self, element_name, element_value, index):
        wd = self.app.wd
        self.app.open_home_page()
        # Select contact by index
        self.get_contact(index).click()
        wd.find_element_by_xpath("//table[@id='maintable']/tbody//td[8]/a/img").click()
        # Update element value found by element name
        wd.find_element_by_name(element_name).click()
        wd.find_element_by_name(element_name).clear()
        wd.find_element_by_name(element_name).send_keys(element_value)
        # Submit update
        wd.find_element_by_name("update").click()
        self.app.open_home_page()

    def modify(self, entry, index):
        wd = self.app.wd
        self.app.open_home_page()
        # Select contact by index
        self.get_contact(index).click()
